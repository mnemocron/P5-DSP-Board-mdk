...
/**
  ...
  * @retval None
  */
void FIR_PROCESSING_F32Process(float32_t *pSrc, float32_t *pDst)
{
	uint32_t i;
  ...
  arm_fir_init_f32(&FIR_F32_Struct, FIR_NUM_TAPS, (float32_t *)&firCoeffs32[0], &firStateF32[0], blockSize);
  /* ----------------------------------------------------------------------
  ** Call the FIR process function for every blockSize samples
  ** ------------------------------------------------------------------- */
  for(i=0; i < numBlocks; i++)
  {
    arm_fir_f32(&FIR_F32_Struct, pSrc + (i * blockSize), pDst + (i * blockSize), blockSize);
  }
}
